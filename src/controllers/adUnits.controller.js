...
// // Create ad unit with file upload
...
//         custom_file_id = uploadResponse.fileId; // Save for DB
...
//       const updateData = {
//         name,
//         code: code || `AD-${Date.now()}`, // Fallback to generated code if not provided
//         ad_type,
//         placement,
//         custom_content,
//         dimensions: dimensions ? JSON.parse(dimensions) : null,
...
//     // Fetch the ad unit to get custom_content and file ID
//     const adUnit = await AdUnit.getAdUnitById(id);
//     if (!adUnit) {
//       return errorResponse(res, 404, "Ad unit not found");
//     }

//     // Log custom content for debugging
...
//           imageKitError
//         );
//         // Continue deletion of DB record even if ImageKit deletion fails
//       }
//     } else {
//       console.log(
...
//     // Delete the ad unit from the database
...
const upsertAdSettings = async (req, res) => {
  try {
    const { id, publisher_id, ad_client, placements, ad_density, ad_format, target_pages } =
      req.body;
    ...
    if (!publisher_id || !ad_client) {
      return errorResponse(
        res,
        400,
        'publisher_id and ad_client are required'
      );
    }
    ...
    if (placements) {
      ...
      const validPlacements = ['header', 'sidebar', 'in_content', 'footer', 'custom'];
      ...
      if (invalidPlacements.length > 0) {
        return errorResponse(
          res,
          400,
          `Invalid placements: ${invalidPlacements.join(', ')}. Must be one of: ${validPlacements.join(', ')}`
        );
      }
